  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
		...
		<el-form-item class="select" label="配送方式" prop="peisongfangshi">
		  <el-select  @change="peisongfangshiChange" clearable v-model="searchForm.peisongfangshi" placeholder="请选择配送方式">
		    <el-option
			v-for="(item,index) in peisongfangshiOptions"
			v-bind:key="index"
			:label="item"
			:value="item">
		    </el-option>
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="菜品价格">
		     <template slot-scope="scope">
                ...
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="diancanshuliang"
                    header-align="center"
		    label="点餐数量">
		     <template slot-scope="scope">
                       {{scope.row.diancanshuliang}}
                     </template>
                </el-table-column>
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="dingdanzonge"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','发布任务') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="peisongrenwuCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发布任务':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','发布任务') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="peisongrenwuCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发布任务':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','发布任务') && contents.tableBtnIcon == 0" type="success" size="mini" @click="peisongrenwuCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发布任务':'' }}</el-button>
    ...
    <peisongrenwu-cross-add-or-update v-if="peisongrenwuCrossAddOrUpdateFlag" :parent="this" ref="peisongrenwuCrossaddOrUpdate"></peisongrenwu-cross-add-or-update>
    ...
    <el-dialog
      ...
      width="800">
        <div id="dingdanzongeChart" style="width:100%;height:600px;"></div>
      <span slot="footer" class="dialog-footer">
...
import peisongrenwuCrossAddOrUpdate from "../peisongrenwu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      peisongrenwuCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    peisongrenwuCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.peisongrenwuCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.peisongrenwuCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.peisongrenwuCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        // xcolumn ycolumn shangjiamingcheng dingdanzonge
        var dingdanzongeChart = this.$echarts.init(document.getElementById("dingdanzongeChart"),'macarons');
        this.$http({
            url: `value/dingdanxinxi/shangjiamingcheng/dingdanzonge`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    yAxis.push(res[i].total.toFixed(2));
                    pArray.push({
                    value: res[i].total,
                    ...
                    // 使用刚指定的配置项和数据显示图表。
                    dingdanzongeChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        dingdanzongeChart.resize();
                    };
                }
            }
        });
    ...
    init () {
          this.peisongfangshiOptions = "商家配送,自提".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        sort: 'id',
          ...
          if(this.searchForm.peisongfangshi!='' && this.searchForm.peisongfangshi!=undefined){
            params['peisongfangshi'] = '%' + this.searchForm.peisongfangshi + '%'
          }
      this.$http({
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          caipintupian: row.caipintupian,
          caipinjiage: row.caipinjiage,
          diancanshuliang: row.diancanshuliang,
          dingdanzonge: row.dingdanzonge,
          ...
          peisongfangshi: row.peisongfangshi,
